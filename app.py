...
try:
    ...
    if len(df) > 0:
        ...
        # HOCANIN İSTEĞİ: Griler Yeşil Oldu
        def renk_ata(row):
            if pd.isna(row['FS']): return 'green'
            if row['FS'] >= 2.0: return 'green'
        ...
        # === SEKME 1: 3D HARİTA (ORİJİNAL - DOKUNULMADI) ===
        with tab1:
            fig3d = go.Figure()
            ...
            if max_x_raw > 150000 or max_y_raw > 150000:
                st.error("⚠️ KORDİNAT UYARISI: Veriniz UTM olarak algılandı ancak saha genişliği 150 km'den fazla çıkıyor! CSV dosyanızda yanlış girilmiş uçuk bir sondaj koordinatı olabilir.")
            ...
            if len(dilim_df['Sondaj_No'].unique()) >= 3: 
                ...
                # Sadece Gri renk, Darkgreen yapıldı. Geçişler ve yapı orijinal.
                colorscale = [[0, 'red'], [0.25, 'orange'], [0.5, 'green'], [1.0, 'darkgreen']]
        ...
        # === SEKME 2: 2D KESİT ÇIKARICI VE LENS (GÜNCELLENDİ) ===
        with tab2:
            st.markdown("### 2D Stratigrafik Kesit Hattı ve Sıvılaşma Lensi")
            ...
            if len(secili_kuyular) >= 2:
                ...
                if len(px) > 4:
                    ...
                    fig2d.add_trace(go.Contour(
                        ...
                        zmin=0.5, zmax=2.0, 
                        opacity=0.45, showscale=True, colorbar=dict(title="FS (Risk)")
                    ))

                # --- KUYU ÇUBUKLARI (ÖN PLAN) ---
                for kuyu in sirali_kuyular:
